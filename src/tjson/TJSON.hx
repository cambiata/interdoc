...
class TJSON {    
	 ...
	 */
	public static function encode(obj:Dynamic, ?style:Dynamic=null, useCache:Bool=true, useKeyQuotes:Bool=true):String{
		var t = new TJSONEncoder(useCache, useKeyQuotes);
...
class TJSONEncoder{
	...
	var cache:Array<Dynamic>;
	var uCache:Bool;
	var useKeyQuotes:Bool;

	public function new(useCache:Bool=true, useKeyQuotes:Bool=true){
		uCache = useCache;
		if (uCache) cache = new Array();
		this.useKeyQuotes = useKeyQuotes;
	}
	...
	private function encodeObject( obj:Dynamic,style:EncodeStyle,depth:Int):String {
		...
		for (field in fields){
			...
			if(vStr!=null){
				...
				var quote = (useKeyQuotes) ? '"' : '';
				buffer.add(quote+field+quote+style.keyValueSeperator(depth)+vStr);
			}
